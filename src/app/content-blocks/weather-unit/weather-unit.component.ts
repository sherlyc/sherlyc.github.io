...
export class WeatherUnitComponent
  ...
  ngOnInit() {
    if (this.runtimeService.isBrowser()) {
      const mediaQuerySubscription = this.mediaQueryService.subscribe(
        (device) => {
          this.deviceType = device;
        }
      );
      const weatherSubscription = this.weatherService.subscribe(
        (location, link) => {
          if (location) {
      ...
      this.subscriptions.push(mediaQuerySubscription, weatherSubscription);
    }
  }
